...
import {
  ...
  clientSdkIds,
  selfHostingServices,
} from "@/components/reference/navigation.references";
import { getFlattenedSections } from "@/components/reference/Reference.generated.singleton";
import { generateOpenGraphImageMeta } from "@/components/reference/openGraph";
// import { BASE_PATH } from "@/lib/constants";

const BASE_PATH = "/docs/reference";
...
export async function generateReferenceMetadata(
  ...
  const { alternates: parentAlternates, openGraph: parentOg } =
    await resolvingParent;
  ...
  if (isClientSdkReference) {
    ...
    const sectionTitle =
      slug.length > 0
        ...
        : undefined;
    const url = [BASE_PATH, "reference", sdkId, maybeVersion, slug[0]]
      .filter(Boolean)
      .join("/");
...
export function normalizeMarkdown(markdownUnescaped: string): string {
  ...
  if (lastIndex > 0) {
    ...
    const escapedRemainingText = remainingText.replace(
      /(?<!\\)([{}])/g,
      "\\$1"
    );
