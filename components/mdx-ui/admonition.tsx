...
>(
  (
    ...
    const typeMapped = variant
      ? admonitionToAlertMapping[variant]
      : admonitionToAlertMapping[type];

    return (
      <Alert
        ref={ref}
        variant={typeMapped === "warning" ? "default" : typeMapped}
        {...props}
        className={cn(
          "not-prose",
          "mb-2",
      ...
      >
        {(showIcon && typeMapped === "warning") ||
        typeMapped === "destructive" ? (
          <WarningIcon />
        ) : showIcon ? (
          <InfoIcon />
        ) : null}
        {label || title ? (
          <>
            ...
            {description && (
              <AlertDescription
                className={props.childProps?.description?.className}
              >
                {description}
              </AlertDescription>
            )}
