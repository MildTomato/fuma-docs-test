...
const Image = ({ src, alt = "", zoomable, ...props }: ImageProps) => {
  ...
  const isLessThanLgBreakpoint = true; //useBreakpoint()
  ...
  return (
    <figure
      className={cn("next-image--dynamic-fill", props.containerClassName)}
    >
      <Component
        {...(zoomable
          ? {
              ZoomContent: ZoomContent,
              zoomMargin: isLessThanLgBreakpoint ? 20 : 80,
            }
          : undefined)}
      >
        <NextImage
      ...
      {props.caption && (
        <figcaption className={cn(getCaptionAlign(props.captionAlign))}>
          {props.caption}
        </figcaption>
      )}
    </figure>
