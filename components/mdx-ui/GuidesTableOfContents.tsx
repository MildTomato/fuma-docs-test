...
import { ExpandableVideo } from "@/components/mdx-ui/ExpandableVideo";
import { proxy, useSnapshot } from "valtio";
import {
  highlightSelectedTocItem,
  removeAnchor,
} from "@/components/mdx-ui/CustomHTMLElements/CustomHTMLElements.utils";
// import { Feedback } from "@/components/mdx-ui/Feedback";
...
const tocRenderSwitch = proxy({
  renderFlag: 0,
  toggleRenderFlag: () =>
    void (tocRenderSwitch.renderFlag = (tocRenderSwitch.renderFlag + 1) % 2),
});
  ...
  useEffect(() => {
    ...
    const timeoutHandle = setTimeout(() => {
      const headings = Array.from(
        document
          .querySelector("#sb-docs-guide-main-article")
      ...
      const newHeadings = headings
        ...
        .filter(
          (x): x is TOCHeader => !!x && !!x.text && !!x.link && !!x.level
        );
              ...
              >
                ...
                >
                  {formatTOCHeader(removeAnchor(item.text)).map((x, index) => (
                    <Fragment key={index}>
                      {x.type === "code" ? (
                        <code className="text-xs border rounded bg-muted">
                          {x.value}
                        </code>
                      ) : (
                        x.value
