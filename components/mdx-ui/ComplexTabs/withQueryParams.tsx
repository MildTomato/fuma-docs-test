import { useSearchParamsShallow } from "@/common/useSearchParamsShallow";
...
import { type TabsProps } from "@/components/mdx-ui/old-tabs";

const isString = (maybeStr: unknown): maybeStr is string =>
...
const withQueryParams =
  ...
  ({
    ...
    const queryTabMaybe =
      queryGroupRef.current && searchParams.get(queryGroupRef.current);
    const queryTab =
      queryTabMaybe && tabIdsRef.current.includes(queryTabMaybe)
        ? queryTabMaybe
        : undefined;
        ...
        setTimeout(() => {
          if (
            ...
            try {
              ...
              if (storedValues === null || typeof storedValues !== "object")
                return;
    ...
    const switchTab = (id: string) => {
      if (queryGroupRef.current) {
        if (
          !searchParams.getAll("queryGroups").includes(queryGroupRef.current)
        ) {
